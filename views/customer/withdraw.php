...
$loggedCustomer=$user[0];
$customer = new Customer($loggedCustomer['name'], $loggedCustomer['email'], $loggedCustomer['password']);
$customerRepo = new CustomerRepository($db, $customer);

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (isset($_POST['amount'])) {
    $amount = (float)trim($_POST['amount']);
  
    $success = $customerRepo->withdraw($amount);

    if ($success) {
      $_SESSION['message'] = "Withdraw successfull";
    } else {
      $_SESSION['message'] = "Withdraw Failed";
    }
  }
}
?>
            ...
            <div class="flex items-center px-2 lg:px-0">
              <div class="hidden sm:block">
                <div class="flex space-x-4">
                  ...
                  <a href="./deposit.php" class="text-white rounded-md py-2 px-3 text-sm font-medium">Deposit</a>
                  <a href="./withdraw.php" class="bg-emerald-700 text-white hover:bg-emerald-500 hover:bg-opacity-75 rounded-md py-2 px-3 text-sm font-medium">Withdraw</a>
                ...
                <div>
                  <button @click="open = !open" type="button" class="flex rounded-full bg-white text-sm focus:outline-none" id="user-menu-button" aria-expanded="false" aria-haspopup="true">
                    ...
                    <span class="inline-flex h-10 w-10 items-center justify-center rounded-full bg-emerald-100">
                      <span class="font-medium leading-none text-emerald-700"><?php echo $_SESSION['user_name'][0];?></span>
                    </span>
                  </button>
                </div>
        ...
        <div class="bg-white rounded-lg p-2">
          ...
          <dl class="mx-auto grid grid-cols-1 gap-px sm:grid-cols-2 lg:grid-cols-4">
            <div class="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-2 bg-white px-4 py-10 sm:px-6 xl:px-8">
              ...
              <?php
                 echo "$".$customer->getCustomerBalance();
                 ?>
              </dd>
            </div>
