...
//var_dump($_SESSION);exit();//['message'] = null;
...
$db = new DB();
//echo "<pre/>";print_r($loggedCustomer);exit();
$user = $db->loadWhere(Customer::getModelName(), array("email" => $email));
$loggedAdmin = $user[0];
$admin = new Admin($loggedAdmin['name'], $loggedAdmin['email'], $loggedAdmin['password']);
$adminRepo = new AdminRepository($db, $admin);
if(isset($_GET['email'])){
  $userMail=$_GET['email'];
  $transactions = $adminRepo->viewTransactions($userMail);
}else{
  $transactions = $adminRepo->viewTransactions();
}

//echo "<pre/>";print_r($transactions);exit();
?>
<php class="h-full bg-gray-100" lang="en">
...
<body class="h-full">
  <div class="min-h-full">
    <div class="bg-sky-600 pb-32">
      <!-- Navigation -->
      <nav class="border-b border-sky-300 border-opacity-25 bg-sky-600" x-data="{ mobileMenuOpen: false, userMenuOpen: false }">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div class="flex h-16 justify-between">
                ...
                <div>
                  <button @click="open = !open" type="button" class="flex rounded-full bg-white text-sm focus:outline-none" id="user-menu-button" aria-expanded="false" aria-haspopup="true">
                    ...
                    <span class="inline-flex h-10 w-10 items-center justify-center rounded-full bg-sky-100">
                      <span class="font-medium leading-none text-sky-700"><?php echo $_SESSION['user_name'][0]; ?></span>
                    </span>
                  </button>
                </div>
            ...
            <div class="-mr-2 flex items-center sm:hidden">
              <!-- Mobile menu button -->
              <button @click="mobileMenuOpen = !mobileMenuOpen" type="button" class="inline-flex items-center justify-center rounded-md p-2 text-sky-100 hover:bg-sky-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-inset focus:ring-sky-500" aria-controls="mobile-menu" aria-expanded="false">
        ...
        <div x-show="mobileMenuOpen" class="sm:hidden" id="mobile-menu">
          <div class="space-y-1 pt-2 pb-3">
            <a href="./customers.php" class="text-white hover:bg-sky-500 hover:bg-opacity-75 block rounded-md py-2 px-3 text-base font-medium">Customers</a>
            <a href="./transactions.php" class="text-white hover:bg-sky-500 hover:bg-opacity-75 block rounded-md py-2 px-3 text-base font-medium">Transactions</a>
          </div>
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                      ...
                      if ($transactions) {
                        foreach ($transactions as $transaction) {
                          ...
                          if ($transaction['transaction_type'] == TransactionType::WITHDRAW->value) {
                            $class = "text-red-600";
                            $amount = "-$" . $transaction['amount'];
                          }
                      ...
                      ?>
                          <tr>
                            ...
                            <td class="whitespace-nowrap px-2 py-4 text-sm font-medium <?php echo $class; ?>">
                            <?php echo $amount; ?>
                            </td>
